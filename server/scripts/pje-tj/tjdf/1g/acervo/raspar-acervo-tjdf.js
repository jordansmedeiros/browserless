/**
 * Raspagem de Processos do Acervo Geral - PJE TJDF 1º Grau
 *
 * FLUXO:
...
// Diretório de saída
const DATA_DIR = 'data/pje/tjdf/acervo';
...
async function fazerLoginSSO(page) {
    ...
    try {
      await page.screenshot({ path: 'debug-login-sso-tjdf.png', fullPage: true });
      console.error('   📸 Screenshot salvo: debug-login-sso-tjdf.png');
    } catch (e) {}
...
async function navegarParaAcervo(page) {
  console.error('🧭 Navegando para Acervo (TJDF)...\n');
  ...
  // TJDF: Verificar se há processos no acervo (estrutura de árvore)
  ...
  const temProcessos = await page.evaluate(() => {
    ...
    // Verificar se existe estrutura de árvore com processos
  ...
  return await page.evaluate((regiao) => {
    ...
    rows.forEach(row => {
      try {
        ...
        // Tentar estrutura padrão primeiro
        if (colunas.length >= 8) {
        ...
        if (!processoInfo.numero && processoInfo.textoCompleto) {
          // Padrão geral PJE
          const numeroMatch = processoInfo.textoCompleto.match(/\d{7}-\d{2}\.\d{4}\.\d{1}\.\d{2}\.\d{4}/);
          if (numeroMatch) {
            processoInfo.numero = numeroMatch[0].trim();
          }

          // Extrair partes (texto com X)
          const partesMatch = processoInfo.textoCompleto.match(/([^|]+?)\s*X\s*([^|]+?)\s*\|/);
          if (partesMatch) {
...
async function obterAgrupadores(page) {
  ...
  const agrupadores = await page.evaluate(() => {
    ...
    // Procurar nós da árvore com processos
 ...
 * Função principal
 */
async function rasparAcervoTJDF() {
  console.error('╔═══════════════════════════════════════════════════════════════════╗');
  console.error('║  RASPAGEM: ACERVO GERAL - PJE TJDF 1º GRAU (Firefox)            ║');
  console.error('╚═══════════════════════════════════════════════════════════════════╝\n');
