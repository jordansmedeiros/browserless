/**
 * Raspagem de Pauta de Audiências - PJE TJCE 1º Grau (SEM CAPTCHA)
 *
 * FLUXO:
...
const PAUTA_URL = 'https://pje.tjce.jus.br/pje1grau/ProcessoAudiencia/PautaAudiencia/listView.seam';

// Diretório de saída
const DATA_DIR = 'data/pje/tjce/pauta';
...
async function navegarParaPauta(page) {
  console.error('🧭 Navegando para Pauta de Audiências (TJCE)...\n');
...
async function aplicarFiltrosPauta(page) {
  ...
  await Promise.all([
    ...
    page.waitForResponse(res => res.url().includes('pje.tjce.jus.br') && res.request().method() === 'POST', { timeout: 45000 })
  ]);
 ...
 * Função principal
 */
async function rasparPautaAudienciasTJCE() {
  console.error('╔═══════════════════════════════════════════════════════════════════╗');
  console.error('║  RASPAGEM: PAUTA DE AUDIÊNCIAS - PJE TJCE 1º GRAU (Firefox)     ║');
  console.error('╚═══════════════════════════════════════════════════════════════════╝\n');
...
rasparPautaAudienciasTJCE().catch(error => {
