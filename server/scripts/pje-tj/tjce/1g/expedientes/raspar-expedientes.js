/**
 * Raspagem de Expedientes - PJE TJCE 1º Grau (SEM CAPTCHA)
 *
 * FLUXO:
...
// Diretório de saída
const DATA_DIR = 'data/pje/tjce/expedientes';
...
/**
 * Faz o login no SSO (TJCE não tem CAPTCHA)
 */
...
async function navegarParaExpedientes(page) {
  console.error('🧭 Navegando para Expedientes (TJCE)...\n');
  ...
  // TJCE: Verificar se há expedientes nos agrupadores (árvore)
  console.error('🔍 Verificando se há expedientes disponíveis...');

  const temExpedientes = await page.evaluate(() => {
    // TJCE usa árvore de jurisdições - procurar nós com expedientes
...
/**
 * Obtém todos os agrupadores da árvore de expedientes (TJCE)
 */
async function obterAgrupadores(page) {
  const agrupadores = await page.evaluate(() => {
    const items = [];

    // TJCE: Procurar nós da árvore com expedientes
...
/**
 * Raspa todos os expedientes da árvore (TJCE)
 */
    ...
    try {
      ...
      while (true) {
        ...
        if (nextButton) {
          ...
          await Promise.all([
            nextButton.click(),
            page.waitForResponse(res => res.url().includes('pje.tjce.jus.br'), { timeout: 30000 })
          ]);
 ...
 * Função principal
 */
async function rasparExpedientesTJCE() {
  console.error('╔═══════════════════════════════════════════════════════════════════╗');
  console.error('║     RASPAGEM: EXPEDIENTES - PJE TJCE 1º GRAU (Firefox)           ║');
  console.error('╚═══════════════════════════════════════════════════════════════════╝\n');
...
rasparExpedientesTJCE().catch(error => {
