/**
 * Script de Teste Automatizado: Multi-TRT Scraping - SEGUNDO GRAU
 *
 * Este script testa o login e raspagem em TODOS os 24 TRTs (SEGUNDO GRAU)
 * para validar que todos seguem o mesmo padrão de URL e autenticação.
 *
 * FUNCIONALIDADES:
 * 1. Testa login em todos os 24 TRTs (2º grau)
 * 2. Verifica se as URLs seguem o padrão esperado
 * 3. Para cada TRT com sucesso, raspa processos pendentes
 * 4. Salva JSONs de auditoria em data/test-multi-trt-2g/
 * 5. Gera relatório detalhado de sucessos e falhas
 *
 * COMO USAR:
 * node --loader ts-node/esm scripts/test-all-trts-2g-scraping.ts
 */
...
const BASE_DATA_DIR = 'data/test-multi-trt-2g';
...
async function criarDiretorios(trt: TRTCode) {
  const trtDir = path.join(BASE_DATA_DIR, trt.toLowerCase(), '2g', 'pendentes');
  await fs.mkdir(trtDir, { recursive: true });
  return trtDir;
}

/**
 * Testa login em um TRT específico (SEGUNDO GRAU)
 */
async function testarLoginTRT(
  ...
  try {
    console.log(`\n${'='.repeat(70)}`);
    console.log(`🧪 Testando ${trt} - ${tribunalInfo.nome} [2º GRAU]`);
    console.log(`   Região: ${tribunalInfo.regiao} (${tribunalInfo.uf})`);
    console.log(`${'='.repeat(70)}`);

    // Obtém configuração do TRT para SEGUNDO GRAU
    const config = await getTribunalConfig(trt, '2g');
    ...
    if (isLoginSuccessful(finalUrl, trtDomain)) {
      ...
      try {
        ...
        const filename = `test-pendentes-2g-${timestamp}.json`;
      ...
      async (p: any) => {
        try {
          const queryString = new URLSearchParams({
            ...
            agrupadorExpediente: p.agrupadorExpediente,
...
async function gerarRelatorio(results: TestResult[]): Promise<TestReport> {
  ...
  const timestamp = new Date().toISOString().replace(/[-:]/g, '').split('.')[0];
  const reportPath = path.join(reportDir, `test-report-2g-${timestamp}.json`);
...
function exibirRelatorio(report: TestReport) {
  console.log('\n' + '═'.repeat(70));
  console.log('📊 RELATÓRIO FINAL - TESTE MULTI-TRT 2º GRAU');
  console.log('═'.repeat(70));
...
async function main() {
  console.log('\n' + '╔' + '═'.repeat(68) + '╗');
  console.log('║' + ' '.repeat(5) + 'TESTE AUTOMATIZADO: MULTI-TRT SCRAPING - 2º GRAU' + ' '.repeat(14) + '║');
  ...
  console.log(`📋 Total de TRTs a testar (2º grau): ${tribunais.length}\n`);
  ...
  for (let i = 0; i < tribunais.length; i++) {
    ...
    console.log(`\n[${i + 1}/${tribunais.length}] Testando ${tribunal.codigo} (2º grau)...`);
